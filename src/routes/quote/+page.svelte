		...
		<div class="bg-white rounded-[20px] shadow-sm p-8 md:p-12 border border-gray-100">
			<form class="space-y-8">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
					...
					<div class="flex flex-col gap-2">
						...
						<div class="relative">
							...
							>
								<option>{$_('quote_page.form.service_options.office')}</option>
								<option>{$_('quote_page.form.service_options.home')}</option>
								<option>{$_('quote_page.form.service_options.sewage')}</option>
								<option>{$_('quote_page.form.service_options.ac')}</option>
								<option>{$_('quote_page.form.service_options.painting')}</option>
							</select>
				...
				<div class="flex justify-end pt-4">
					<button 
						type="submit" 
						class="bg-[#15562E] hover:bg-[#0f4223] text-white font-medium px-8 py-3.5 rounded-lg transition-colors shadow-sm"
					>
						{$_('quote_page.form.submit')}
					</button>
				</div>
			</form>
		</div>
